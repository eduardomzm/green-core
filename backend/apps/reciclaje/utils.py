...
from .models import Deposito, RankingMensual


def generar_ranking_mensual():
    ...
    if RankingMensual.objects.filter(anio=anio, mes=mes).exists():
        return
    ...
    total_piezas = depositos.aggregate(total=Sum('cantidad'))['total'] or 0

    material_top = top_materiales[0]['material__nombre'] if top_materiales else "N/A"

    RankingMensual.objects.create(
